...
function TeamsUserIsInterestedInForEvent({ event }: TeamsUserIsInterestedInForEventProps) {
    ...
    const fetchLeads = useCallback(async () => {
        ...
        try {
            const response = await axios.get(
                `${BASE_URL}/api/events/${event.id}/leadsForTeams`,
                {
                    headers: {
    ...
    if (!event) {
        return (
            <div className="p-4 text-center text-gray-500">
                Please select an event to view teams you're interested in.
            </div>
        );
    }
    ...
    return (
        <div className="p-4">
            <h3 className="text-lg font-semibold mb-4">
                Teams you're interested in for {event.name}
            </h3>
...
export default TeamsUserIsInterestedInForEvent;